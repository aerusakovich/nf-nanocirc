...
Confidence scoring (same system as add_isoform_confidence.py):
    bsj_score       : number of tools in group (1-4)
    isoform_score   : tools with confirmed isoform overlap — min 1
    overlap_score   : avg pairwise fraction → 1-4 (1 if no pairs)
    final_score     : sum (3-12) → Low/Medium/High/Very High
...
def compute_group_scores(group, nodes_with_isoform, node_pair_fracs):
    """
    Compute all score components for a group.
    """
    tools_in_group = set(group.keys())
    bsj_score = len(tools_in_group)  # component 1

    # component 2: tools with isoform support — min 1
    ...
    return isoform_confidence, isoform_score, overlap_score, final_score, final_cat


def write_outputs(groups, active_tools, nodes_with_isoform,
                  node_pair_fracs, sample, outdir):
    union_lines      = []
    inter_lines      = []
    union_conf_lines = []
    inter_conf_lines = []
    ...
    header_cols = (
        ...
        + ["bsj_score", "isoform_confidence", "isoform_score",
           "overlap_score", "final_score", "final_cat"]
    )
    ...
    for group in sorted(groups, key=lambda g: sorted(
        ...
        (isoform_confidence, isoform_score, overlap_score,
         final_score, final_cat) = compute_group_scores(
            group, nodes_with_isoform, node_pair_fracs)

        bsj_score  = len(tools_in_group)
...
def main():
    ...
    write_outputs(groups, active_tools, nodes_with_isoform,
                  node_pair_fracs, args.sample, args.outdir)
