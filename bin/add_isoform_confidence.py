...
Adds three confidence components and a final score to the confidence TSV
produced by merge_circrna.py.

Components:
    bsj_score       : number of tools detecting the BSJ (1-4)
    isoform_score   : number of tools with isoform support — minimum 1
                      (1 tool always agrees with itself)
    overlap_score   : average pairwise spliced-length overlap fraction
                      across all overlapping pairs, converted to 1-4:
                        < 25%  → 1
                        25-50% → 2
                        50-75% → 3
                        75-100%→ 4
...
def final_score_to_cat(score):
    """Convert final score (3-12) to category."""
    if score <= 3:
        return "Low"
    elif score <= 6:
        return "Medium"
...
def compute_scores(bsj_id, bsj_confidence, tool_flags, all_pair_results):
    ...
    bsj_score = bsj_confidence  # 1-4

    # ── component 2: isoform_score ────────────────────────────────────────
    # minimum 1: 1 tool always agrees with itself
    isoform_score = max(1, isoform_confidence)
    ...
    final_cat   = final_score_to_cat(final_score)

    return (isoform_confidence, bsj_score, isoform_score,
            overlap_score, final_score, final_cat, pair_fracs)
    ...
    with open(args.confidence) as fh:
        ...
        for lineno, line in enumerate(fh, 1):
            ...
            (isoform_confidence, bsj_score, isoform_score,
             overlap_score, final_score, final_cat, pair_fracs) = compute_scores(
                bsj_id, bsj_confidence, tool_flags, all_pair_results)
            ...
            out_lines.append(
                ...
                "\t".join(frac_values + [
                    ...
                    str(overlap_score), str(final_score), final_cat
                ])
            )
